...
def test_online_item_scores_at_the_top_of_its_attendance_ability_axis():
    dto = build_dto(item(is_online=True, title="AI hackathon"))
    assert dto.relevance == 8 + 25 + 15  # unknown cost + attendance-ability maxed (online) + core topic fit
...
def test_funding_signals_from_the_second_look_feed_the_score():
    ...
    assert dto.relevance >= 17 + 13  # partial-funding affordability, core topic fit, at minimum
